...
from carsus.util import HDFWriterMixin
...
class RegressionData:
    ...
    def sync_hdf_store(self, carsus_module, update_fname=True):
        """
        Synchronizes the HDF store with the regression data.

        Parameters
        ----------
        carsus_module : object
        ...
        if self.enable_generate_reference:
            ...
            with pd.HDFStore(self.fpath, mode="w") as store:
                carsus_module.to_hdf(store, overwrite=True)
            pytest.skip(
