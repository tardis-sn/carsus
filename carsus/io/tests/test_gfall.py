...
HISTORICAL_GFALL_URL = (
    "https://raw.githubusercontent.com/tardis-sn/carsus-data-kurucz/"
    "c62cddf4b5861323f60b5190f78621089e7832ca/linelists/gfall/gfall.dat"
)
...
def test_gfall_reader_preserves_historical_o_iv_fine_structure(
    historical_o_iv_gfall_fname,
):
    reader = GFALLReader(
        fname=historical_o_iv_gfall_fname,
        unique_level_identifier=["energy", "j", "label"],
    )
...
@pytest.mark.remote_data
def test_historical_gfall_regression():
    reader = GFALLReader(
        "O 3",
        fname=HISTORICAL_GFALL_URL,
        unique_level_identifier=["energy", "j", "label"],
    )

    assert len(reader.gfall_raw) == 564949
    assert len(reader.gfall) == 563247

    levels = reader.levels.loc[(8, 3)].reset_index()
    o_iv_levels = levels.loc[
        (levels["label"] == "2p3 *2D")
        & levels["energy"].isin([255155.9, 255184.9]),
        ["energy", "j", "label"],
    ].sort_values(["energy", "j"])
