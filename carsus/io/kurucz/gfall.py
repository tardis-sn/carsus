...
class GFALLIngester(object):
    ...
    def __init__(
        ...
        if (
            self.data_source.data_source_id is None
    ...
    def get_lvl_index2id(self, ion):
        ...
        q_ion_lvls = self.session.query(
            Level.level_id.label("id"), Level.level_index.label("index")
        ).filter(and_(Level.ion == ion, Level.data_source == self.data_source))
    ...
    def ingest_levels(self, levels=None):
        ...
        if self.ions is not None:
            levels = (
                levels.reset_index()
                .join(self.ions, how="inner", on=["atomic_number", "ion_charge"])
            ...
            for index, row in ion_levels.iterrows():
                ...
                ion.levels.append(
                    Level(
                        ...
                        energies=[
                            LevelEnergy(
                                quantity=row["energy"] * u.Unit("cm-1"),
                                method=row["method"],
                                data_source=self.data_source,
                            )
                        ],
    ...
    def ingest_lines(self, lines=None):
        ...
        for ion_index, ion_lines in lines.groupby(
            ...
            for index, row in ion_lines.iterrows():
                ...
                medium = (
                    MEDIUM_VACUUM
                    if row["wavelength"] <= GFALL_AIR_THRESHOLD
                    else MEDIUM_AIR
                )
                ...
                line = Line(
                    ...
                    wavelengths=[
                        LineWavelength(
                            quantity=row["wavelength"] * u.nm,
                            medium=medium,
                            data_source=self.data_source,
                        )
                    ],
                    gf_values=[
                        LineGFValue(quantity=row["gf"], data_source=self.data_source)
                    ],
                )
